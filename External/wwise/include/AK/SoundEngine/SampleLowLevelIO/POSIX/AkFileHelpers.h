...
public:
		...
		)
	{
		...
		if ( eResult == AK_Success )
		{
			...
			int fd = fileno(out_fileDesc.hFile);
			if( fstat(fd, &buf) != 0)
			{
				return AK_UnknownFileError;
			}
			if (buf.st_size == 0 && (in_eOpenMode == AK_OpenModeRead || in_eOpenMode == AK_OpenModeReadWrite))
			{
				AKPLATFORM::OutputDebugMsgV("AkFileHelpers Warning: File %s (File no. %d) was opened for reading but has size 0.\n", in_pszFileName, fd);
			}
