    ...
    )
{
	...
	{
		io_bSyncOpen = true;
		AKRESULT eResult = CAkMultipleFileLocation::Open(in_pszFileName, in_eOpenMode, in_pFlags, true/*Always overlapped in DeferredIO*/, out_fileDesc);
    ...
    )
{
	...
	{
		io_bSyncOpen = true;
		eResult = CAkMultipleFileLocation::Open(in_fileID, in_eOpenMode, in_pFlags, true /*Always overlapped in DeferredIO*/, out_fileDesc);
		if ( eResult == AK_Success )
		{
			out_fileDesc.uSector			= 0;
			out_fileDesc.deviceID			= m_deviceID;
