...
inline const std::vector<Transition>& ResourceStateMachine::GetTransitions() const
{
	return transitions;
}
...
inline void ResourceStateMachine::EraseState(unsigned int id)
{
	...
	{
		transitions.erase(transitions.begin() + transitionid);
	}
...
inline void ResourceStateMachine::EraseTransition(unsigned int id)
{
	...
	transitions.erase(transitions.begin() + id);
}