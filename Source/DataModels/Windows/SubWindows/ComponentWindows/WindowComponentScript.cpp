...
void WindowComponentScript::DrawWindowContents()
{
	...
	if (!scriptObject)
	{
				...
				finalLabel.c_str(), &current_item, constructors.data(), static_cast<int>(constructors.size()), 5))
		{
			...
			LOG_INFO("{} SELECTED, drawing its contents.", script->GetConstructName());
		}
	...
	if (ImGui::Button(finalLabel.c_str()))
	{
		LOG_INFO("%s REMOVED, showing list of available scripts.", script->GetConstructName().c_str());
...
void WindowComponentScript::AddNewScriptToProject(const std::string& scriptName)
{
	...
	{
		LOG_INFO("That name is already in use, please use a different one");
		return;
	}
	...
	LOG_INFO("New script %s created", scriptName.c_str());
}
