...
void WindowResources::DrawResource(const std::weak_ptr<Resource>& resource, std::vector<UID>& resourcesToDelete)
{
	...
	if (asShared)
	{
		...
		ImGui::BeginDisabled();
		bool isLoaded = asShared->IsLoaded();
		ImGui::Checkbox(("Is loaded##" + uidString).c_str(), &isLoaded);
		ImGui::EndDisabled();

		if (ImGui::Button(("Load resource##" + uidString).c_str()))
		{
			asShared->Load();
		}
		if (ImGui::Button(("Unload resource##" + uidString).c_str()))
		{
			asShared->Unload();
		}
		if (ImGui::Button(("Delete resource##" + uidString).c_str()))
		{
