	...
	if (App->IsOnPlayMode())
	{
		...
		if (isKinematic)
		{
			...
			if (useRotationController)
			{
				...
				Quat nextRotation(
					q.x + deltaRotation.x, q.y + deltaRotation.y, q.z + deltaRotation.z, q.w + deltaRotation.w);
...
void ComponentRigidBody::ApplyTorque()
{
	...
	else
	{
		...
		Quat nextRotation(q.x + deltaRotation.x, q.y + deltaRotation.y, q.z + deltaRotation.z, q.w + deltaRotation.w);
		nextRotation.Normalize();
