...
{
	const unsigned int program = App->GetModuleProgram()->GetProgram();
	...
	if(material) 
	{
		...
		std::shared_ptr<ResourceTexture> texture = App->GetModuleResources()->
		...
		texture = App->GetModuleResources()->RequestResource<ResourceTexture>(material->GetSpecularUID()).lock();
		if (texture)
		{
			if (!texture->IsLoaded())
			{
		...
		texture = App->GetModuleResources()->RequestResource<ResourceTexture>(material->GetNormalUID()).lock();
		if (texture)
		{
			if (!texture->IsLoaded())
			{
		...
		float3 viewPos = App->GetModuleEngineCamera()->GetPosition();
...
void ComponentMaterial::LoadOptions(Json& meta)
{
	...
	std::shared_ptr<ResourceMaterial> resourceMaterial = 
		App->GetModuleResources()->RequestResource<ResourceMaterial>(uidMaterial).lock();
	...
	else 
	{
		...
		if (resourceExists) 
		{
			uidMaterial = App->GetModuleResources()->ImportResource(path);
			resourceMaterial = App->GetModuleResources()->RequestResource<ResourceMaterial>(uidMaterial).lock();
			SetMaterial(resourceMaterial);
		}
	}
...
void ComponentMaterial::UnloadTextures()
{
	if(material)
	{
		std::shared_ptr<ResourceTexture> texture = 
			App->GetModuleResources()->RequestResource<ResourceTexture>(material->GetDiffuseUID()).lock();
		if (texture)
		{
			texture->Unload();
		}
		...
		texture = App->GetModuleResources()->RequestResource<ResourceTexture>(material->GetOcclusionrUID()).lock();
		if (texture)
		{
			texture->Unload();
		}
...
void ComponentMaterial::UnloadTexture(TextureType textureType)
{
	if (material)
	{
		...
		case TextureType::DIFFUSE:
			texture = App->GetModuleResources()->RequestResource<ResourceTexture>(material->GetDiffuseUID()).lock();
			if (texture)
			{
				texture->Unload();
			}
