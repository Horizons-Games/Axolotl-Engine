...
void MaterialImporter::Save(const std::shared_ptr<ResourceMaterial>& resource, char*& fileBuffer, unsigned int& size)
{
	...
	App->fileSystem->Save(metaPath.c_str(), buffer.GetString(), static_cast<unsigned int>(buffer.GetSize()));
...
void MaterialImporter::Load(const char* fileBuffer, std::shared_ptr<ResourceMaterial> resource)
{
	...
	if (texturesUIDs[2] != 0)
	{
		resource->SetOcclusion(App->resources->SearchResource<ResourceTexture>(texturesUIDs[2]));
	}
	...
	if (texturesUIDs[3] != 0)
	{
		switch (*shaderType)
		{
			case 0:
				...
				resource->SetMetallic(App->resources->SearchResource<ResourceTexture>(texturesUIDs[3]));

				break;

			case 1:

				resource->SetSpecular(App->resources->SearchResource<ResourceTexture>(texturesUIDs[3]));
