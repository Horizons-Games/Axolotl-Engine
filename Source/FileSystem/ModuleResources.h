...
const std::shared_ptr<R> ModuleResources::RequestResource(const std::string path)
{
	...
	if (App->GetModule<ModuleFileSystem>()->Exists(metaPath.c_str()))
	{
		...
		long long libTime =
			App->GetModule<ModuleFileSystem>()->GetModificationDate((libraryPath + GENERAL_BINARY_EXTENSION).c_str());
		if (assetTime <= libTime)
		{
			std::string fileName =
				App->GetModule<ModuleFileSystem>()->GetFileName(libraryPath + GENERAL_BINARY_EXTENSION);
			...
			std::shared_ptr<Resource> resource = CreateResourceOfType(
				uid, App->GetModule<ModuleFileSystem>()->GetFileName(assetPath), assetPath, libraryPath, type);
